...
the configured local Harness, negotiates its capabilities and advertised
session modes, and retains one ACP conversation per logical scope — the Room
conversation plus each Task's own session — waking the right one for each
addressed room turn. The runtime itself
