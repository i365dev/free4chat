...
function parseArgs(argv) {
  ...
  const alias = {
    "base-url": "baseUrl", room: "room", name: "name", target: "target",
    mode: "mode", "listen-seconds": "listenSeconds",
    "wait-grant-seconds": "waitGrantSeconds", "dump-dir": "dumpDir", "go-bin": "goBin",
  };
  for (let i = 2; i < argv.length; i++) {
    ...
    if (!key) { console.error(`unknown flag ${k}`); return null; }
    args[key] = argv[++i];
  }
  return args;
}
...
function fail(reason) {
  console.log(`RESULT FAIL last_stage=${lastStage} reason=${JSON.stringify(reason)}`);
  process.exitCode = 1;
  throw new Error(reason);
}
const httpLog = (entry) => appendFileSync(join(dumpDir, "http-trace.jsonl"), JSON.stringify(entry) + "\n");
...
async function sfuCall(method, path, body) {
  ...
  httpLog({ ts: new Date().toISOString(), method, path, status: res.status, ms: Date.now() - t0, req: body ?? null, respText: text.slice(0, 20000) });
  let json = null;
  try { json = JSON.parse(text); } catch { /* non-JSON error bodies are evidence too */ }
...
async function mcpCall(toolName, toolArgs) {
  ...
  const text = await res.text();
  httpLog({ ts: new Date().toISOString(), method: `MCP ${toolName}`, path: "/mcp", status: res.status, ms: Date.now() - t0, req: body, respText: text.slice(0, 20000) });
  if (!res.ok) throw new Error(`mcp ${toolName} -> HTTP ${res.status}: ${text.slice(0, 300)}`);
  let payload = null;
  ...
  if (payload.error) throw new Error(`mcp ${toolName} rpc error: ${JSON.stringify(payload.error)}`);
  const parsed = JSON.parse(payload.result.content[0].text);
  if (payload.result.isError || parsed.error) throw new Error(`mcp ${toolName}: ${parsed.error ?? "tool error"}`);
  return parsed;
}
...
const joinPayload = await mcpCall("join_room", { roomId: args.room, name: args.name });
if (joinPayload.error) fail(`join_room: ${joinPayload.error}`);
writeFileSync(join(dumpDir, "join-result.json"), JSON.stringify(joinPayload, null, 2));
const handle = joinPayload.participantHandle;
const creds = decodeHandle(handle);
const myId = creds.participantId;
if (!creds.room || !myId || !creds.participantToken) fail("handle decode produced incomplete credentials");
  ...
  while (!stopHeartbeat) {
    try {
      const ev = await Promise.race([
        mcpCall("wait_for_events", { participantHandle: handle, cursor, timeoutSeconds: 8 }),
    ...
    } catch { /* transient; lease window is 90s */ }
  }
})();
...
stage("grant-wait", `polling room_info for the Meeting Notes grant (max ${args.waitGrantSeconds}s)`);
console.log(`
>> ACTION REQUIRED <<
  ...
  while (!granted && Date.now() < grantDeadline) {
    ...
    if (notes.active === true && notes.agentParticipantId === myId) {
      if (state.meetingNotesMediaAvailable === false) {
        fail("grant is ours but meetingNotesMediaAvailable=false (AGENT_MEDIA_ENABLED off)");
      }
      granted = true;
...
const goBin = args.goBin || join(pathDirname(fileURLToPath(import.meta.url)), "pion-cloudflare");
const goProc = spawn(goBin, ["-dump-dir", dumpDir], { stdio: ["pipe", "pipe", "inherit"] });
if (!goProc || !goProc.stdin) fail(`failed to spawn go child at ${goBin}`);
goProc.on("error", (e) => console.error(`[driver] go child error: ${e.message}`));
if (goProc.stderr) goProc.stderr.on("data", (d) => process.stderr.write(`[go] ${d}`));
...
goProc.stdout.on("data", (chunk) => {
  for (const line of chunk.toString().split("\n")) {
    ...
    if (!trimmed) continue;
    let msg;
    try { msg = JSON.parse(trimmed); } catch { console.log(`[driver] unparseable go stdout: ${trimmed.slice(0, 120)}`); continue; }
    if (msg.ev) {
      events.push(msg);
      for (let i = eventWaiters.length - 1; i >= 0; i--) {
        const w = eventWaiters[i];
        if (w.pred(msg)) { eventWaiters.splice(i, 1); w.resolve(msg); }
      }
      continue;
    }
...
function goSend(cmd, timeoutMs = 45000) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => { pending.delete(cmd.id); reject(new Error(`go op=${cmd.op} timed out`)); }, timeoutMs);
    pending.set(cmd.id, (msg) => { clearTimeout(timer); msg.ok ? resolve(msg) : reject(new Error(msg.error ?? "go op failed")); });
...
try {
  ...
  try {
    const r = await sfuCall("POST", "/api/sfu/agent-session", {
      room: args.room, participantId: myId, token: creds.participantToken,
    ...
    console.log(`[C] native-offer rejected (${err.message}); trying blank-session contract`);
    const r = await sfuCall("POST", "/api/sfu/agent-session", {
      room: args.room, participantId: myId, token: creds.participantToken,
    });
    sessionId = r.sessionId;
  ...
  if (!initialDesc) {
    const transport = await sfuCall("POST", "/api/sfu/datachannels/establish", {
      ...
      console.log(`[C] establish-with-offer rejected (${err.message}); server-offer fallback`);
  ...
  const appliedInitial = await goSend({
    op: "apply-remote", type: initialDesc.type, sdp: initialDesc.sdp,
  });
  ...
  const candidates = (media.participants ?? []).filter(
    (p) => p.sessionId && (!args.target || p.name.toLowerCase().includes(args.target.toLowerCase()))
  );
  let picked = null;
  for (const p of candidates) {
    const audio = (p.tracks ?? []).find((t) => t.kind === "audio" && t.trackName);
    if (audio) { picked = { participant: p, trackName: audio.trackName }; break; }
  }
  if (!picked) fail(`no live human audio source found (target=${JSON.stringify(args.target)})`);
  console.log(`[F] selected "${picked.participant.name}" track=${picked.trackName}`);
  ...
  const sub = await sfuCall("POST", "/api/sfu/tracks", {
    room: args.room, participantId: myId, token: creds.participantToken,
    sessionId,
    tracks: [{ location: "remote", sessionId: picked.participant.sessionId, trackName: picked.trackName }],
  });
  if (!sub.sessionDescription) fail("tracks/new returned no sessionDescription");
  const expectedMid = sub.tracks?.[0]?.mid;
  if (!expectedMid) fail("tracks/new returned no usable mid");
  console.log(`[G] remote-type=${sub.sessionDescription.type} mid=${expectedMid}`);
  ...
  const appliedSub = await goSend({
    op: "apply-remote", type: sub.sessionDescription.type, sdp: sub.sessionDescription.sdp,
  });
  ...
  const trackEv = await waitForEvent(
    ...
    15000, `OnTrack(mid=${expectedMid})`
  );
  const t = trackEv.track;
  console.log(`[H] ontrack kind=${t.kind} mime=${t.mime} clock=${t.clockRate} ch=${t.channels} pt=${t.payloadType} ssrc=${t.ssrc}`);

  console.log(`>> SPEAK NOW: human mic audio should flow for ${args.listenSeconds}s <<`);
  ...
  bytes = packets * 76;
  console.log(`[I] rtp_packets=${packets} counts=${JSON.stringify(finalCounts)}`);
  if (!(packets > 0)) fail(`stage I: ReadRTP produced 0 packets (mid=${t.mid})`);

  console.log(`RESULT PASS rtp_packets=${packets} codec=${t.mime}/${t.clockRate}/${t.channels} pt=${t.payloadType} ssrc=${t.ssrc} mid=${t.mid}`);
  console.log("issue #100 Phase 1 acceptance criterion SATISFIED: TrackRemote.ReadRTP() > 0");
} catch (err) {
  console.log(`RESULT FAIL last_stage=${lastStage} reason=${JSON.stringify(String(err.message ?? err))}`);
  process.exitCode = 1;
} finally {
  stopHeartbeat = true;
  goProc.stdin.write(JSON.stringify({ id: 999999, op: "close" }) + "\n");
  setTimeout(() => goProc.kill("SIGKILL"), 2000);
  try { mcpCall("leave_room", { participantHandle: handle }).catch(() => {}); } catch {}
  heartbeat.catch(() => {});
}
