...
function SidebarNav({
  ...
  return (
    <nav aria-label="Documentation" className="space-y-6 font-mono text-sm">
      <Link
        href={DOCS_ROOT_PATH}
        className={
            ...
            <li key={link.href}>
              <a
                ...
                {...(link.href.startsWith("http")
                  ...
                  : {})}
                className="block text-emerald-600 hover:text-emerald-300"
              >
                {link.title}
              </a>
            </li>
    ...
    <div className="flex min-h-screen flex-col font-mono text-emerald-100">
      ...
      <header className="border-b border-emerald-900/60">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2 text-sm">
            <Link
              href="/"
              className="font-semibold text-emerald-200 hover:text-emerald-400"
            >
              Free4Chat
            </Link>
          ...
          <button
            ...
            className="rounded-none border border-emerald-900 px-2 py-1 text-xs text-emerald-500 md:hidden"
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-1 gap-10 px-4 py-8">
        ...
        <main className="min-w-0 flex-1">
          {menuOpen && (
            <div className="mb-8 border-b border-emerald-900/60 pb-8 md:hidden">
              {nav}
            </div>
          )}
          <article className="prose prose-invert max-w-none font-mono text-emerald-300/90 prose-headings:text-emerald-100 prose-a:text-emerald-400 prose-strong:text-emerald-50 prose-code:text-emerald-300 prose-code:before:content-none prose-code:after:content-none">
            <ReactMarkdown remarkPlugins={[remarkGfm]} components={components}>
              {source}
            </ReactMarkdown>
          </article>
          <div className="mt-10 flex items-center justify-between gap-4 border-t border-emerald-900/60 pt-6 font-mono text-sm">
            {prev ? (
              <Link
                href={prev.href}
            ...
            {next ? (
              <Link
                href={next.href}
                className="text-right text-emerald-500 hover:text-emerald-200"
              >
                {next.title} &rarr;
              </Link>
            ) : (
