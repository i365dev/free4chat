...
describe("robots.txt", () => {
  ...
  it("points to the sitemap", () => {
    ...
    expect(robots).toMatch(
      /Sitemap:\s*https:\/\/www\.free4\.chat\/apps-sitemap\.xml/
    )
  })
})
...
describe("sitemap.xml", () => {
  ...
  it("leaves App discovery and its sitemap to the Lab control plane", () => {
    expect(existsSync(join(ROOT, "src/pages/apps/index.tsx"))).toBe(false)
    expect(existsSync(join(ROOT, "src/pages/apps/whiteboard.tsx"))).toBe(false)
    expect(locs).not.toContain("https://www.free4.chat/apps")
    expect(locs).not.toContain("https://www.free4.chat/apps/whiteboard")
  })
