...
export function isValidPermissionTaskRequestId(value: string): boolean {
  return REQUEST_ID_PATTERN.test(value)
}

export interface PermissionRequestInput {
  // This must be a fresh random Room-correlation id generated by the resident
  // Runtime for each ACP permission request. It is not the Harness ACP
  // JSON-RPC request id; the Runtime owns that mapping and generation.
  requestId?: unknown
  toolCall?: unknown
  options?: unknown
  expiresInMs?: unknown
  // Optional canonical collaboration Task correlation. RoomSession validates
  // it against the retained Task projection; it is not a generic client
  // selected scope.
...
export function validatePermissionRequest(
  ...
  if (input.taskRequestId !== undefined) {
    if (
      typeof input.taskRequestId !== "string" ||
      !isValidPermissionTaskRequestId(input.taskRequestId)
    )
...
export function pendingPermissionRequestIsEquivalent(
  ...
  return (
    permissionRequestFingerprint(record.event, record.taskRequestId) ===
    permissionRequestFingerprint(event, taskRequestId)
  )
}
