...
export function useRealtimeKitChatRoom(
  roomName: string,
  nickName: string,
  ...
  }, [roomName, nickName, roomType, enableBot, enabled, initMeeting])

  useEffect(() => {
    if (!enabled || !meeting || joinedMeetingRef.current === meeting) return
