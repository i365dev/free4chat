...
// TCGETS ioctl succeeds on terminals and fails with ENOTTY on files,
...
func termDisableEcho(file *os.File) (func() error, error) {
	...
	if _, _, errno := syscall.Syscall(syscall.SYS_IOCTL, fd, syscall.TCSETS, uintptr(unsafe.Pointer(&termios))); errno != 0 {
		return nil, errors.New("terminal echo control is unavailable")
	}
	return func() error {
		if _, _, errno := syscall.Syscall(syscall.SYS_IOCTL, fd, syscall.TCSETS, uintptr(unsafe.Pointer(&original))); errno != 0 {
