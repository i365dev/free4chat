...
- Current phase: **OBSERVE / DOGFOOD**. The core Task/Runtime stabilization cycle is complete and the latest released Runtime is the supported configuration; issue #51 is the canonical product north star, and focused core work is opened only when real usage proves a gap. Do not present already-closed planning issues as current gates or blockers.
...
`/mcp` is a stateless MCP v2 endpoint built with `createMcpHandler` from `agents/mcp/server` and `McpServer` from `@modelcontextprotocol/server`. It exposes nineteen tools — `room_info`, `read_room_context`, `join_room`, `create_room`, `wait_for_events`, `send_text`, `update_capabilities`, `update_runtime_host`, `send_collab_request`, `send_collab_response`, `send_collab_result`, `send_attachment`, `publish_surface`, `clear_surface`, `read_surface`, `read_attachment`, `publish_live_view`, `publish_generated_app`, and `leave_room`; `app/public/agent.md` is the canonical contract. The opaque participant handle is a bearer capability containing the room and Agent participant credentials; never log, display, or send it anywhere except the Free4Chat MCP endpoint.
...
`publish_generated_app` stores one bounded, sandboxed, Task-scoped App as Room-owned state: at most a 48 KiB bundle and 16 KiB shared state, at most four Generated Apps per Room, an empty `manifest.networkOrigins`, and one publication per Task (a changed valid bundle keeps the same `appInstanceId` and increments `bundleRevision`). The Room validates and owns the canonical publication. Keep public network capability intentionally constrained: no generic app hosting, no localhost/tunnel publishing, no arbitrary backend runtime.

The MCP/text protocol itself is text-only and always will be: no MCP tool ever returns or accepts SFU session IDs, participant tokens, track IDs, or DataChannel IDs. `room_info` returns sanitized participant/capability data and bounded committed Live Transcript context where present; it never returns ordinary chat history, tokens, connection nonces, provider proofs, SFU session IDs, track IDs, or DataChannel IDs. `read_room_context` is the separately bounded, authenticated, read-only historical observation surface; Room event and Live Transcript sequence domains remain separate. Transcript visibility is Room context, not an ordinary message and not a Harness wakeup; explicit addressing remains the activation boundary. `wait_for_events` is an event-read and lease heartbeat that returns immediately by default (a held HTTP long-poll is an explicit, bounded, per-environment opt-in via `MCP_LONGPOLL_ENABLED`, because holding a request keeps the Room awake and billable); it returns sanitized event context with per-Agent `addressed` metadata. Human browser-to-browser images remain DataChannel transfers; a bounded temporary image copy may exist for Agent inspection, and explicit Room attachments support jpeg/png/webp plus text/plain, text/markdown, text/csv, application/json, and text/yaml (≤768KB). Text attachments are returned decoded as UTF-8 in the tool result. Do not replace this with polling loops, queues, R2, or a second Durable Object.

For resident participation, the canonical Go runtime (`agent/`) owns the opaque participant handle, cursor, lease heartbeat, reconnect/rejoin, bounded sanitized event buffer, attachment reads, and Harness wakeup. One Free4Chat participant represents the runtime across many model turns. The Harness receives only sanitized room context and returns response text; it never receives the handle/token/cursor, SFU credentials, or raw media, and does not call the Free4Chat MCP tools directly. When older shared context is genuinely needed, it may invoke the Runtime-mediated local `free4chat-agent context read` command; that narrow observation path has no Room lifecycle or mutation authority. The Runtime speaks generic ACP v1 through its launcher registry: Hermes, OpenCode, Codex, Claude, Pi, and a trusted local custom ACP command all use the same retained-session adapter. Do not auto-approve privileged Harness tools.
...
- **Runtime version support policy.** Free4Chat currently supports only the latest released Agent Runtime. The Web/Room and the Runtime evolve together, so older Runtime versions may not understand current private Room↔Runtime controls, may implement older Task/Harness semantics, and may lack current features or bug fixes. Do not add backward-compatibility negotiation, old private-control fallbacks, compatibility shims, minimum-version comparison, or version-specific branches unless an explicit future issue changes this product policy. This rule exists to avoid paying compatibility complexity while the Agent/Task protocol is still evolving rapidly; it replaces any per-feature mixed-version reasoning.
