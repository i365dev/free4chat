...
- **Runtime-owned streaming STT, no TTS or persistence.** Meeting Notes media authorization remains #87's responsibility. Streaming STT is an optional Runtime-owned/BYOK downstream capability layered on top of the same subscribe-only ingress; raw audio and transcripts remain outside Worker/DO state and are never written to R2, KV, or DO storage. TTS and outbound voice remain future work.
